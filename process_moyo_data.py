...
def determine_floor_height_and_contacts(body_joint_seq, fps):
    ...
    if all_static_foot_heights.shape[0] > 0:
        ...
        for cur_label in all_labels:
            ...
            cur_clust_inds = np.unique(all_static_inds[
                                           clustering.labels_ == cur_label])  # inds in the original sequence that correspond to this cluster
            if VIZ_PLOTS:
        ...
        if DISCARD_TERRAIN_SEQUENCES:
            ...
            for cluster_root_height, cluster_height, cluster_size in zip(cluster_root_heights, cluster_heights,
                                                                         cluster_sizes):
    ...
    # hand contacts
    left_hand_contact = detect_joint_contact(body_joint_seq, 'leftHand', floor_height, CONTACT_VEL_THRESH,
                                             CONTACT_ANKLE_HEIGHT_THRESH)
    right_hand_contact = detect_joint_contact(body_joint_seq, 'rightHand', floor_height, CONTACT_VEL_THRESH,
                                              CONTACT_ANKLE_HEIGHT_THRESH)
    contacts[:, SMPL_JOINTS['leftHand']] = left_hand_contact
    contacts[:, SMPL_JOINTS['rightHand']] = right_hand_contact

    # knee contacts
    left_knee_contact = detect_joint_contact(body_joint_seq, 'leftLeg', floor_height, CONTACT_VEL_THRESH,
                                             CONTACT_ANKLE_HEIGHT_THRESH)
    right_knee_contact = detect_joint_contact(body_joint_seq, 'rightLeg', floor_height, CONTACT_VEL_THRESH,
                                              CONTACT_ANKLE_HEIGHT_THRESH)
...
def compute_align_from_right(body_right):
    world2aligned_angle = np.arccos(body_right[:, 0] / (
                np.linalg.norm(body_right[:, :2], axis=1) + 1e-8))  # project to world x axis, and compute angle
    body_right[:, 2] = 0.0
    world2aligned_axis = np.cross(body_right, np.array([[1.0, 0.0, 0.0]]))

    world2aligned_aa = (world2aligned_axis / (
                np.linalg.norm(world2aligned_axis, axis=1)[:, np.newaxis] + 1e-8)) * world2aligned_angle[:, np.newaxis]
...
def process_seq(data_paths):
    ...
    while process_inds[0] < num_frames:
        ...
        body = get_body_model_sequence(smplh_path, gender, process_inds[1] - process_inds[0],
                                       pose_body[sidx:eidx], pose_hand[sidx:eidx], betas, root_orient[sidx:eidx],
                                       trans[sidx:eidx])
    ...
    if SAVE_VELOCITIES:
        ...
        # body pose
        pose_body_mat = axisangle2matrots(pose_body.reshape(num_frames, len(SMPL_JOINTS) - 1, 3)).reshape(
            (num_frames, len(SMPL_JOINTS) - 1, 3, 3))
...
def main(config):
    ...
    all_dataset_dirs = [os.path.join(config.moyo_root, f) for f in sorted(os.listdir(config.moyo_root)) if
                        f[0] != '.']
    all_dataset_dirs = [f for f in all_dataset_dirs if os.path.isdir(f)]
    print('Found %d available datasets from raw MOYO data source.' % (len(all_dataset_dirs)))
    all_dataset_names = [f.split('/')[-1] for f in all_dataset_dirs]
    print(all_dataset_names)

    # requested datasets
    dataset_dirs = [os.path.join(config.moyo_root, f) for f in config.datasets]
    ...
    for data_dir, data_name in zip(dataset_dirs, dataset_names):
        if not os.path.exists(data_dir):
            print('Could not find dataset %s in available raw MOYO data!' % (data_name))
            return
        ...
        cur_subject_dirs = [f for f in sorted(os.listdir(data_dir)) if
                            f[0] != '.' and os.path.isdir(os.path.join(data_dir, f))]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--moyo-root', type=str, default='./data/moyo_raw',
                        help='Root directory of raw MOYO dataset.')
    parser.add_argument('--datasets', type=str, nargs='+', default=ALL_DATASETS,
                        help='Which datasets to process. By default processes all.')
    parser.add_argument('--out', type=str, default='./data/moyo_processed',
                        help='Root directory to save processed output to.')
    parser.add_argument('--smplh-root', type=str, default='./body_models/smplh',
                        help='Root directory of the SMPL+H body model.')
